...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def do_reboot(self):
        if self.client is not None:
            if QMessageBox.Ok == QMessageBox.warning(
                self, "Confirm", "Reboot?", buttons=QMessageBox.Ok | QMessageBox.Cancel
            ):
    ...
    def initialize_leds(self):
        ...
        def update_preview():
            ...
            palette.setColor(
                self.backgroundRole(),
                QColor(self.leds_r.value(), self.leds_g.value(), self.leds_b.value()),
            )
    ...
    def initialize_motors(self):
        ...
        self.motors_pan.sliderReleased.connect(
            lambda: self.client.send_command("set_pan", angle=self.motors_pan.value())
        )
        self.motors_tilt.sliderReleased.connect(
            lambda: self.client.send_command("set_tilt", angle=self.motors_tilt.value())
        )
        self.motors_pan_torque_on.clicked.connect(
            lambda: self.client.send_command("set_pan_torque", control=True)
        )
        self.motors_pan_torque_off.clicked.connect(
            lambda: self.client.send_command("set_pan_torque", control=False)
        )
        self.motors_tilt_torque_on.clicked.connect(
            lambda: self.client.send_command("set_tilt_torque", control=True)
        )
        self.motors_tilt_torque_off.clicked.connect(
            lambda: self.client.send_command("set_tilt_torque", control=False)
        )

    def initialize_behaviours(self):
        def enable_look_around(checked):
            self.client.send_command(
                "enable_behaviour", name="look_around", control=checked
            )
        ...
        def enable_conversation(checked):
            self.client.send_command(
                "enable_behaviour", name="conversation", control=checked
            )
        ...
        def enable_photographer(checked):
            self.client.send_command(
                "enable_behaviour", name="photographer", control=checked
            )
    ...
    def initialize_audio(self):
        ...
        def play_sound():
            self.client.send_command(
                "play_sound", name=self.audio_sound_list.currentText()
            )
        ...
        def update_screen():
            def f():
                self.client.send_command(
                    "set_screen",
                    image=""
                    if self.screen_image_list.currentText() == "<None>"
                    else self.screen_image_list.currentText(),
                    video=""
                    if self.screen_video_list.currentText() == "<None>"
                    else self.screen_video_list.currentText(),
        ...
        def upload_image():
            ...
            if filename:
                url = (
                    ...
                    + self.client.ip
                    + ":"
                    + str(self.client.multimedia_port)
                    + "/images"
                )
    ...
    def update(self):
        if self.client is not None:
            ...
            try:
                self.battery.setText("Battery: %.2f V" % self.client.battery)
                ...
                self.audio_speech.setText(
                    self.client.recognized_speech
                    if self.client.recognized_speech is not None
                    else "<speech>"
                )
